...
namespace  ProductApi.Infra.Data.Repositories
{
    ...
    {
        public async Task<IEnumerable<Core.Entities.RegisterVehicleVersion>?> GetAllAsync(RecordStatusEnum recordStatus)
        {
            var query =
                    await Task.FromResult(
                        GenerateQuery(
                            filter: (filtr => filtr.Status.Equals((int)recordStatus)),
                            orderBy: item => item.OrderBy(y => y.RegisterVehicleVersionId)));
